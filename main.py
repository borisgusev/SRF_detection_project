...
for img_path in srf:
    ...
    n = 7
    segmented_image, cluster_labels = utls.segmentation(image_gray_gaus, nclust=n)
    ...
    for i in range(len(y_edges)):
        if (y_edges[i]!=0 or x_edges[i]!=0 or y_edges[i]!=segmented_image_with_edges.shape[0] or x_edges[i]!=segmented_image_with_edges.shape[1]):
            segmented_image_with_edges[y_edges[i],x_edges[i]]=(1,0,0) # possibly will help......
